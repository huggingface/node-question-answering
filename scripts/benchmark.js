...
const DATA = [
  ...
  },
  {
    ...
    questions: [
      ...
      "For what did Maria Skłodowska-Curie achieved international recognition?"
    ]
  },
  {
    context: `
...
(async () => {
  ...
  await qaClient.predict("dummy", "dummy");
  ...
  for (const type of Object.keys(times)) {
    console.log(
      `average inference time (${type} texts):`,
      average(times[type].inference)
    );
    console.log(`average total time (${type} texts):`, average(times[type].total));
  }
})();
...
function average(numbers) {
  const raw = numbers.reduce((acc, val) => acc + val, 0) / numbers.length;
