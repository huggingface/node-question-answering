...
async function downloadModel() {
  ...
  if (!(await exists(`${assetsDir}/distilbert`))) {
    ...
    await new Promise((resolve, reject) => {
      https.get("https://s3.amazonaws.com/models.huggingface.co/bert/distilbert-base-uncased-distilled-squad-384-saved_model.tar.gz", res => {
  ...
  if (!(await exists(`${assetsDir}/${vocabFile}`))) {
    ...
    await new Promise((resolve, reject) => {
      https.get("https://s3.amazonaws.com/models.huggingface.co/bert/bert-base-uncased-vocab.txt", res => {
        const bar = new ProgressBar('[:bar] :percent :etas', {
          width: 30,
          total: parseInt(res.headers['content-length'], 10)
        });
...
async function npmPublish() {
  ...
  shell.cp("-r", [distPath, "package.json", "README.md", "LICENSE", "./scripts"], buildPath);
  ...
  await fs.promises.writeFile(`${buildPath}/package.json`, JSON.stringify(npmPackageJson, null, 2));
